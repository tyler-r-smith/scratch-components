...
export class q1s_fileinput extends q1s_input {
    constructor(obj){
        ...
        this.constructCallback = (elem, options) =>{
            elem.label = elem.shadowRoot.children[1].children[0];
            elem.imageDisplay = elem.shadowRoot.children[1].children[1];
            elem.input = elem.shadowRoot.children[1].children[2];
            elem.helperText = elem.shadowRoot.children[1].children[3];
            elem.returnValue = (v) =>  elem.parent.returnValue.call(elem.input, undefined, v);

            elem.input.FileData = {};
            elem.input.FileData.file = 'nothing here :(';
            elem.input.onchange = () => {
                console.log(elem.input.file);
